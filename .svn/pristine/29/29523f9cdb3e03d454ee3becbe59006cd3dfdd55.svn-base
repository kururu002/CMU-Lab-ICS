...
/* $end mallocmacros */
/*put pointer*/
#define PUT_PREV(bp, val) (*(unsigned*)(bp)=(val))
#define PUT_NEXT(bp, val) (*((unsigned*)(bp)+1)=(val))



/* Global variables */
static char *heap_listp = 0;  /* Pointer to first block */  
static void *frep=0;//free list pointer
...
static void *coalesce(void *bp) 
{
    ...
    if(prev_alloc && next_alloc) {add_free(bp);}//case1
...
static void delete_free(void *bp){
    ...
    if(next) PUT_PREV(next,prev);
}

static void add_free(void *bp){
    if(frep) PUT_PREV(frep,bp);
    PUT_NEXT(bp,frep);
    PUT_PREV(bp,0);
    frep = bp;
}
...
/* $end mmfirstfit */
    ...
    for(bp=frep;bp!=0;bp=GET_NEXT(bp)){
